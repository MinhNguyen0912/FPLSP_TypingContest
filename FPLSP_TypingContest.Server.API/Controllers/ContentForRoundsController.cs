...
namespace FPLSP_TypingContest.Server.API.Controllers
{
    ...
    [ApiController]
    public class ContentForRoundsController : ControllerBase
    {
        private readonly IContentForRoundServices _Services;

        public ContentForRoundsController(IContentForRoundServices Services)
        {
            _Services = Services;
        }

        [HttpPost]
        public async Task<IActionResult> AddAsync([FromBody] ContentForRoundCreateVM request)
        {
            if (request == null)
            {
                return BadRequest();
        ...
        [HttpPut]
        [Route("{id}")]
        public async Task<IActionResult> UpdateAsync(Guid id, [FromBody] ContentForRoundUpdateVM request)
        {
            if (request == null)
            {
                return BadRequest();
